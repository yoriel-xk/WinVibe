...
- fingerprint **不是**幂等键。它只用于 §1.6 决策序里的两类一致性校验：
  - 同 `approval_id` 重试：fingerprint 必须与首次相同，否则 `duplicate_id`；
  - hookcli 重试自身请求时由 hookcli 端校验 payload 未被改写。
  「不同 approval_id + 同 fingerprint + 当前有 active」**仍**返回 `busy_another_active`，**不**幂等合并。
...
pub enum EnqueueError {
    ...
    /// 内部不变量保护：MVP-1 的 cached 走 FIFO 淘汰，正常路径不会触达；
    /// 一旦触发视为 bug，HTTP 层映射到 500 internal_error + diagnostic，
    /// **不**对外暴露专属稳定 code。
    StoreFull,
}
...
- `expires_at_mono_ms = created_mono_ms + approval_ttl_ms`，TTL 默认 300 秒（可配置）。
...
| 409 | `busy_another_active` | 已有 active 审批 | hookcli fail-closed，不写 stdout，exit 2 |
| 404 | `approval_not_found` | poll/decide 引用了未知 approval_id（既不在 by_id 也不在 cached） | hookcli/HUD 视为 fatal，exit 2（HUD 提示后清理本地引用） |
...
hookcli 进程退出码语义**与决策无关**，仅表达「是否安全把控制交还 Claude Code」：

- `0`：协议层成功——服务端给出确定结果（200 Decision），hookcli 已经把对应的 Claude Code hook JSON 写到 stdout。Claude Code 据 stdout JSON 决定 allow/block。其中 TimedOut / Cancelled 的 approve/block 由 `timeout_action` 配置决定（默认 `deny` → block）。
- `2`：fail-closed——**无法从服务端拿到确定结果时一律 block**。命中场景：网络不可达 / 连接拒绝 / 读写超时 / TLS 失败 / 401 / 403 / 404 / 400 / 413 / 422 / 409 duplicate_id / 409 busy_another_active / 500 / 503 shutting_down / 协议反序列化失败 / 配置加载失败。不写 stdout JSON。
...
| 200 TimedOut | 跟随 `timeout_action` 配置：`deny`(默认) → `{ "decision": "block", "reason": "winvibe: timed_out" }`；`approve` → `{ "decision": "approve", "reason": "winvibe: timed_out" }` | 0 |
| 200 Cancelled | 跟随 `timeout_action` 配置：`deny`(默认) → `{ "decision": "block", "reason": "winvibe: cancelled:<reason>" }`；`approve` → `{ "decision": "approve", "reason": "winvibe: cancelled:<reason>" }` | 0 |
| 409 busy_another_active | 不写 stdout | 2 |
| 网络错误 / 连不上 server / 任何超时 | 不写 stdout | 2 (fail-closed) |
| 401/403/404/400/413/422/409 duplicate_id/500/503 | 不写 stdout | 2 |

stderr 始终承载诊断信息；diagnostic JSONL 是结构化版本，stderr 是人类可读版本。**网络错误一律 fail-closed**——历史草稿曾出现过 fail-open 写法，已废弃，不得再引入。「TimedOut」是服务端**主动决策**返回的终态，不是网络超时；二者必须区分。
...
- `200` Approved / Denied / TimedOut / Cancelled（决策路径全覆盖；TimedOut/Cancelled 的 approve/block 跟随 `timeout_action` 配置，断言默认 `deny` → block；断言对应 hookcli stdout JSON 与 exit 0）
- `202` 短轮询超时 → hookcli 复用 id 二次 poll，无 exit
- `400 invalid_request`
- `401 unauthorized`、`403 origin_forbidden`
- `404 approval_not_found`（poll/decide 一个从未存在过的 approval_id）
- `409 busy_another_active`（hookcli fail-closed，不写 stdout，exit 2）
...
pub struct WinvibeConfig {
    ...
    pub timeout_action: TimeoutAction, // Deny (默认) | Approve
}
...
pub struct RawWinvibeConfig {
    ...
    pub timeout_action: String,       // "deny" (默认) | "approve"
}
...
/// 纯加载（无引导），供 hookcli 与测试使用。auth_token 缺失是
...
- `load_or_init_config_app` 是 app 入口的唯一推荐路径；`load_config_strict` 供 hookcli 与测试使用。hookcli **不**参与 token 生成（首启 token 必须由 app 落盘后 hookcli 才能读到）。hookcli 拿到 `MissingAuthToken` 时 → exit 2 + stderr 提示先启动 app。
