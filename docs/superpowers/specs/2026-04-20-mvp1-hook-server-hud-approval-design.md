...
| 503 | `store_full` | ApprovalStore 容量上限触达（异常路径，MVP-1 单 active 下仅可能因 cached 写入异常或 race） | hookcli fatal，exit 2；服务端同时打 diagnostic |

### 4.2 hookcli exit code 与 stdout 协议

hookcli 进程退出码语义**与决策无关**，仅表达「是否安全把控制交还 Claude Code」：

- `0`：协议层成功——服务端给出确定结果（200 Decision 或 409 busy_another_active），hookcli 已经把对应的 Claude Code hook JSON 写到 stdout。Claude Code 据 stdout JSON 决定 allow/block。
- `2`：fail-closed——**无法从服务端拿到确定结果时一律 block**。命中场景：网络不可达 / 连接拒绝 / 读写超时 / TLS 失败 / 401 / 403 / 404 / 400 / 413 / 422 / 409 duplicate_id / 500 / 503（含 shutting_down 与 store_full） / 协议反序列化失败 / 配置加载失败。不写 stdout JSON。
...
- `503 store_full`（fault injection 把 max_cached 调到 0 并塞满）
...
- bind 地址在**配置加载期**校验——`RawWinvibeConfig::validate()` 解析 `bind: String` → `IpAddr`，再断言 `IpAddr::is_loopback()`，否则 `ConfigValidationError::BindNotLoopback`。配置加载失败的进程退出码统一为 78 `EX_CONFIG`，**不**绑端口、**不**写 audit。运行时层不再做第二次 bind 安全校验，单点真相在 §5.5。
